...
export default function Form({ onCompleted }) {
  const initState = {
    spreadsheetId: "1ReLJrnUTfLucy4l_ugcr7gmlUx0DDPkuzCEbyPSAo0A",
    range: "A1:M166",
    verses: 4,
  };
  ...
  return (
    <Card as="form" onSubmit={(e) => e.preventDefault()}>
      ...
      <Input sx={{ mb: 3 }} name="spreadsheetId" value={state.spreadsheetId} onChange={handleChange} />
      <Label htmlFor="range">range:</Label>
      <Input sx={{ mb: 3 }} name="range" value={state.range} onChange={handleChange} />
      <Label htmlFor="verses">verses:</Label>
      <Input sx={{ mb: 3 }} name="verses" value={state.verses} onChange={handleChange} />
      <br />
      <div sx={{ display: "flex" }}>
        {loading ? (
          <Spinner sx={{ mx: "auto" }} />
          ...
          <>
            ...
            <Button sx={{ flex: 1, ml: 2 }} variant="secondary" onClick={() => setState(initState)}>
              Reset
            </Button>
          </>
        )}
